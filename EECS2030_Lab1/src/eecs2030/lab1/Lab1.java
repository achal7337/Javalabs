...
public class Lab1 {
	...
	public static double pressureAltitude(double pressure) {
		double x = pressure/1013.25 ; 
		
		return 145366.45*(1-Math.pow(x, 0.190284));
	}
	...
	public static String alternatingCaps(String s) {
		...
		for (int i= 0; i <= s.length()-1;i++) {
			char ch = s.charAt(i);
				if(i%2 ==0) {
					result= nString.append(Character.toUpperCase(ch));
				}
				else
				{
					result = nString.append(Character.toLowerCase(ch));
				}
			}
